...
class Review:
    ...
    def find_by_id(cls, id):
        """Return a Review instance corresponding to the table row matching the specified primary key (id).
        Return None if no row with the specified id exists."""
        sql = """
            SELECT *
            FROM reviews
            WHERE id = ?
    ...
    def update(self):
        """Update the year, summary, and employee_id columns for the 'reviews' table row
        corresponding to the current Review instance's id."""
        sql = """
            UPDATE reviews
